...
Timer::Timer(uint64_t ms, std::function<void()> cb, bool recurring, TimerManager* manager)
    ...
    ,m_manager(manager)
{
    m_next = shuai::GetCurrentMS() + m_ms; 
}
...
{
    m_previouseTime = shuai::GetCurrentMS();
}
...
uint64_t TimerManager::getNextTimer()
{
    ...
    if(now_ms >= next->m_next)   // 表示任务该执行了
        return 0;
    else 
...
{
    // SHUAI_LOG_DEBUG(SHUAI_LOG_ROOT()) << "insert m_timers";
    auto it = m_timers.insert(val).first;  // 获取插入val元素的迭代器
