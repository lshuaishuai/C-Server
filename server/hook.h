...
extern "C"  // 告诉编译器使用 C 的链接方式，而不是 C++ 的。
{

// sleep
// 这里定义了一个新的类型 sleep_fun，它是一个指向函数的指针，函数接受一个 unsigned int 参数表示秒数，并返回一个 unsigned int。这个函数类型与 POSIX 标准中的 sleep 函数相匹配
// extern: 这个关键字用于声明一个变量，该变量的定义在其他文件中。这意味着 sleep_f 是一个外部变量，可能在其他源文件中被定义。
// sleep_fun sleep_f;: 这里声明了一个名为 sleep_f 的变量，其类型为之前定义的 sleep_fun，即指向一个接受 unsigned int 参数并返回 unsigned int 的函数的指针。
typedef unsigned int (*sleep_fun)(unsigned int seconds);
extern sleep_fun sleep_f;  // 老的定义 sleep_fun指向原来的sleep
...
typedef int (*ioctl_fun)(int d, unsigned int long request, ...);
