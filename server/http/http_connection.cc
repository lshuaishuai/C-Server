...
HttpConnection::HttpConnection(Socket::ptr sock, bool owner)
    ...
    // ,m_createTime(shuai::GetCurrentMS())
{}

HttpConnection::~HttpConnection()
{
    SHUAI_LOG_INFO(g_logger) << "HttpConnection::~HttpConnection()";
}
...
HttpResponse::ptr HttpConnection::recvResponse()
{
    ...
    if(client_parser.chunked)
    {
        ...
        do{
            ...
            SHUAI_LOG_INFO(g_logger) << "content_len = " << client_parser.content_len;
...
HttpResult::ptr HttpConnection::DoRequest(HttpMethod method, Uri::ptr uri, uint64_t timeout_ms, const std::map<std::string, std::string>& headers, const std::string& body)
{
    ...
    for(auto& i : headers)
    {
        ...
        if(!has_host && (i.first.c_str(), "host") == 0)
        {
...
HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req, Uri::ptr uri, uint64_t timeout_ms)
{
    ...
    if(rt < 0)
        return std::make_shared<HttpResult>((int)HttpResult::Error::SEND_SOCKET_ERROR, nullptr, "send requesr socket error errno = : " + std::to_string(errno) + " errstr = " + std::string(strerror(errno)));
...
{
    uint64_t now_ms = shuai::GetCurrentMS();
    ...
    if(!ptr)
    {
        ...
        if(!addr)
        {
            SHUAI_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if(!sock)
        {
            SHUAI_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr))
        {
            SHUAI_LOG_ERROR(g_logger) << "socket connect fail: " << *addr;
            return nullptr;
        }
...
{
    ++ptr->m_request;
    if(!ptr->isConnected() || (ptr->m_createTime + pool->m_maxAliveTime) >= shuai::GetCurrentMS() || (ptr->m_request >= pool->m_maxRequest))
    {
        delete ptr;
        --pool->m_total;     
        return;
